...
# Este archivo se copia tal cual dentro de la skill, que se puede instalar en
# cualquier parte, así que no puede dar por hecho que está dentro del
# repositorio. Busca el catálogo en tres saltos, y el orden importa: DENTRO del
# repositorio manda siempre el del repositorio, nunca la copia que viaje con la
# skill — si no, se trabajaría contra una copia vieja sin enterarse.
def raiz_repo(desde=None):
    """La raíz del repositorio a partir del directorio actual, o None."""
    d = os.path.abspath(desde or os.getcwd())
    while True:
        if os.path.isfile(os.path.join(d, 'generador', 'variables_fap.json')):
            return d
...
def buscar_catalogo():
    candidatos = [
        os.environ.get('FAP_CATALOGO'),                                  # 1 · lo que diga quien manda
        os.path.join(RAIZ, 'generador', 'variables_fap.json'),           # 2 · el repositorio, si el script vive en él
    ]
    repo = raiz_repo()
    if repo:
        candidatos.insert(1, os.path.join(repo, 'generador', 'variables_fap.json'))
    candidatos += [
        os.path.join(AQUI, 'references', 'variables_fap.json'),          # 3 · la copia que viaja con la skill
        os.path.join(os.path.dirname(AQUI), 'references', 'variables_fap.json'),
    ]
    for c in candidatos:
        if c and os.path.isfile(c):
            return os.path.abspath(c)
    return os.path.join(RAIZ, 'generador', 'variables_fap.json')


CATALOGO = buscar_catalogo()
# Las plantillas solo existen dentro del repositorio; fuera, --check no tiene
# qué mirar y lo dice en vez de recorrer el disco de quien lo ejecute.
RAIZ_PLANTILLAS = raiz_repo() or RAIZ
PLANTILLAS = os.path.join(RAIZ_PLANTILLAS, 'generador', 'plantillas')
...
def descubrir_plantillas(raiz=None):
