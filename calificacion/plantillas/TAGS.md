...
- **Matriz de documentos** — `{#docs}{formulario}{detalle}{res}{/docs}`, envuelta además en
  `{#docsGate}` … `{/docsGate}` (el título "Matriz Cumple o No cumple — entrega de documentos"
  y la tabla completa). Cuando el proceso califica los documentos "Con puntaje" (panel 2,
  selector "Documentos habilitantes") la herramienta manda `docsGate:false` y todo ese bloque
  desaparece del acta — no hace falta tocar nada a mano en Word.
